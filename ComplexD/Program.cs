...
namespace ComplexD
{
    class Program
    {
        struct Complex
        {
            ...
            internal static double Pow(Complex value, int power)
            {
                double r = Math.Sqrt(Math.Pow(value.Real, 2) + Math.Pow(value.imaginary, 2));
                double teta = Math.Atan(value.Imaginary / value.Real);
                double z= power * r *( Math.Sin(teta * power)+Math.Cos(teta * power));
                return z;
            }
        }
